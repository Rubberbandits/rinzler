...
fn test_serving_subscribe_ok_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
                ...
                let call = RuntimeCall::ParatensorModule(ParatensorCall::serve_axon { version, ip, port, ip_type, protocol, placeholder1, placeholder2});
...
fn test_serving_ok() {
	new_test_ext().execute_with(|| {
                ...
                assert_ok!(ParatensorModule::serve_axon(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version, ip, port, ip_type, protocol, placeholder1, placeholder2));
...
fn test_serving_set_metadata_update() {
	new_test_ext().execute_with(|| {
                ...
                assert_ok!(ParatensorModule::serve_axon(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version2, ip2, port2, ip_type2, protocol2, placeholder12, placeholder22));
                let neuron = ParatensorModule::get_axon_info( &hotkey_account_id );
...
fn test_axon_serving_rate_limit_exceeded() {
	new_test_ext().execute_with(|| {
                ...
                assert_eq!(ParatensorModule::serve_axon(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version, ip, port, ip_type, protocol, placeholder1, placeholder2), Err(Error::<Test>::ServingRateLimitExceeded.into()) );    
        });
}
...
fn test_prometheus_serving_subscribe_ok_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
                ...
                let port: u16 = 128;
                let ip_type: u8 = 4;
                let call = RuntimeCall::ParatensorModule(ParatensorCall::serve_prometheus{ version, ip, port, ip_type});
                assert_eq!(call.get_dispatch_info(), DispatchInfo {
                        weight: frame_support::weights::Weight::from_ref_time(0),
                        class: DispatchClass::Normal,
                        pays_fee: Pays::No
...
fn test_prometheus_serving_ok() {
	new_test_ext().execute_with(|| {
                ...
                add_network(netuid, tempo, modality);
                register_ok_neuron( netuid, hotkey_account_id, 66, 0);
                assert_ok!(ParatensorModule::serve_prometheus(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id),version, ip, port, ip_type));
...
fn test_prometheus_serving_set_metadata_update() {
	new_test_ext().execute_with(|| {
                ...
                let port2: u16 = port + 1;
                let ip_type2: u8 = 6;
                assert_ok!(ParatensorModule::serve_prometheus(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version2, ip2, port2, ip_type2));
                let neuron = ParatensorModule::get_prometheus_info(  &hotkey_account_id );
                assert_eq!(neuron.ip, ip2);
                assert_eq!(neuron.version, version2);
                assert_eq!(neuron.port, port2);
...
fn test_prometheus_serving_rate_limit_exceeded() {
	new_test_ext().execute_with(|| {
                ...
                assert_ok!(ParatensorModule::serve_prometheus(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version, ip, port, ip_type));    
                ParatensorModule::set_serving_rate_limit( 1 );
                assert_eq!(ParatensorModule::serve_prometheus(<<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id), version, ip, port, ip_type), Err(Error::<Test>::ServingRateLimitExceeded.into()) );    
        });
}
