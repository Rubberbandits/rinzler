...
fn test_add_stake_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::ParatensorModule(ParatensorCall::add_stake{hotkey, amount_staked});
...
fn test_add_stake_ok_no_emission() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id), hotkey_account_id, 10000));
...
fn test_add_stake_err_signature() {
	new_test_ext().execute_with(|| {
		...
		let result = ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::none(), hotkey_account_id, amount);
...
fn test_add_stake_not_registered_key_pair() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id), hotkey_account_id, amount), Err(Error::<Test>::NotRegistered.into()));
	});
}
...
fn test_add_stake_err_neuron_does_not_belong_to_coldkey() {
	new_test_ext().execute_with(|| {
		...
		let result = ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(other_cold_key), hotkey_id, 1000);
...
fn test_add_stake_err_not_enough_belance() {
	new_test_ext().execute_with(|| {
		...
		let result = ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey_id), hotkey_id, 60000);
...
fn test_remove_stake_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
		...
		let call = RuntimeCall::ParatensorModule(ParatensorCall::remove_stake{hotkey, amount_unstaked});
		assert_eq!(call.get_dispatch_info(), DispatchInfo {
			weight: frame_support::weights::Weight::from_ref_time(0),
			class: DispatchClass::Normal,
			pays_fee: Pays::No
...
fn test_remove_stake_ok_no_emission() {
	new_test_ext().execute_with(|| {
		...
		// Do the magic
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey_account_id), hotkey_account_id, amount));
...
fn test_remove_stake_err_signature() {
	new_test_ext().execute_with(|| {
		...
		let result = ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::none(), hotkey_account_id, amount);
		assert_eq!(result, DispatchError::BadOrigin.into());
	});
}
...
fn test_remove_stake_err_hotkey_does_not_belong_to_coldkey() {
	new_test_ext().execute_with(|| {
		...
		// Perform the request which is signed by a different cold key
		let result = ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(other_cold_key), hotkey_id, 1000);
		assert_eq!(result, Err(Error::<Test>::NonAssociatedColdKey.into()));
	});
}
...
fn test_remove_stake_no_enough_stake() {
	new_test_ext().execute_with(|| {
		...
		let result = ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey_id), hotkey_id, amount);
...
fn test_delegate_stake_division_by_zero_check(){
    new_test_ext().execute_with(|| { 
        ...
        assert_ok!(ParatensorModule::become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey), hotkey) );
...
fn test_full_with_delegating() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 60000), Err(Error::<Test>::NotEnoughBalanceToStake.into()));
		...
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100), Err(Error::<Test>::NotRegistered.into()));
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100), Err(Error::<Test>::NotRegistered.into()));

		// Cant remove either.
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 10), Err(Error::<Test>::NotRegistered.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 10), Err(Error::<Test>::NotRegistered.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 10), Err(Error::<Test>::NotRegistered.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 10), Err(Error::<Test>::NotRegistered.into()));

		// Neither key can become a delegate either because we are not registered.
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100), Err(Error::<Test>::NotRegistered.into()));
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100), Err(Error::<Test>::NotRegistered.into()));
		...
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 100), Err(Error::<Test>::NonAssociatedColdKey.into()));
		assert_eq!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 100), Err(Error::<Test>::NonAssociatedColdKey.into()));
		...
		assert_eq!( ParatensorModule::get_stake_for_coldkey_and_hotkey( &coldkey0, &hotkey0 ), 0 );
		assert_eq!( ParatensorModule::get_stake_for_coldkey_and_hotkey( &coldkey0, &hotkey0 ), 0 );
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100) );
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 100) );
		...
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 10), Err(Error::<Test>::NonAssociatedColdKey.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 10), Err(Error::<Test>::NonAssociatedColdKey.into()));
		...
		// Set take to be 0.
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 0), Err(Error::<Test>::NonAssociatedColdKey.into()));
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 0), Err(Error::<Test>::NonAssociatedColdKey.into()));

		// Become delegates all is ok.
		assert_ok!( ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 10) ); 
		assert_ok! (ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 10) );
		assert!( ParatensorModule::hotkey_is_delegate( &hotkey0 ) );
		assert!( ParatensorModule::hotkey_is_delegate( &hotkey1 ) );

		// Cant become a delegate twice.
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 1000), Err(Error::<Test>::AlreadyDelegate.into()));
		assert_eq!(ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 1000), Err(Error::<Test>::AlreadyDelegate.into()));
		...
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 200) );
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 300) );
		assert_eq!( ParatensorModule::get_stake_for_coldkey_and_hotkey( &coldkey0, &hotkey0 ), 200 );
		...
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100000), Err(Error::<Test>::NotEnoughStaketoWithdraw.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 100000), Err(Error::<Test>::NotEnoughStaketoWithdraw.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 100000), Err(Error::<Test>::NotEnoughStaketoWithdraw.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 100000), Err(Error::<Test>::NotEnoughStaketoWithdraw.into()));

		// unstaking is ok.
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey0, 100) );
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey1, 100) );
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey1, 100) );
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey0, 100) );
		...
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey2), hotkey2, 1000) );
		assert_ok!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey2), hotkey2, 100) );
		assert_eq!( ParatensorModule::get_stake_for_coldkey_and_hotkey( &coldkey2, &hotkey2 ), 900 ); 
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey2, 10), Err(Error::<Test>::NonAssociatedColdKey.into()));
		assert_eq!(ParatensorModule::remove_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey2, 10), Err(Error::<Test>::NonAssociatedColdKey.into()));

		// Lets make this new key a delegate with a 50% take.
		assert_ok!( ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey2), hotkey2, u16::MAX/2) ); 

		// Add nominate some stake.
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey2, 1000) );
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey2, 1000) );
		assert_ok!(ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey2), hotkey2, 100) );
		...
		assert_ok!( ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey3), hotkey3, 1000) );
		assert_ok!( ParatensorModule::do_become_delegate(<<Test as Config>::RuntimeOrigin>::signed(coldkey3), hotkey3, u16::MAX ) ); // Full take. 
		assert_ok!( ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey0), hotkey3, 1000) );
		assert_ok!( ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey1), hotkey3, 1000) );
		assert_ok!( ParatensorModule::add_stake(<<Test as Config>::RuntimeOrigin>::signed(coldkey2), hotkey3, 1000) );
