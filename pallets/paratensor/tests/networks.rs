...
fn test_add_network_dispatch_info_ok() { 
        ...
        let modality = 0;
        let tempo: u16 = 13;
        let call = RuntimeCall::ParatensorModule(ParatensorCall::sudo_add_network{netuid, tempo, modality});
...
fn test_clear_min_allowed_weight_for_network() { 
	...
	register_ok_neuron( 1, 55, 66, 0);
        ...
        assert_ok!(ParatensorModule::do_remove_network(<<Test as Config>::RuntimeOrigin>::root(), netuid));
...
fn test_remove_difficulty_for_network() {
	new_test_ext().execute_with(|| {
        ...
        add_network(netuid, tempo, 0);
	register_ok_neuron( 1, 55, 66, 0);
        assert_ok!(ParatensorModule::sudo_set_difficulty(<<Test as Config>::RuntimeOrigin>::root(), netuid, difficulty));
...
fn test_network_set_emission_ratios_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::ParatensorModule(ParatensorCall::sudo_set_emission_values{ netuids, emission });
...
fn test_network_set_emission_ratios_fail_summation() {
	new_test_ext().execute_with(|| {
        ...
        add_network(1, 0, 0);
        add_network(2, 0, 0);
        assert_eq!( ParatensorModule::sudo_set_emission_values(<<Test as Config>::RuntimeOrigin>::root(), netuids, emission ), Err(Error::<Test>::InvalidEmissionValues.into()) );
});}
...
fn test_network_set_emission_invalid_netuids() {
	new_test_ext().execute_with(|| {
        ...
        let emission: Vec<u64> = vec![ 100000000, 900000000 ]; 
        add_network(1, 0, 0);
        assert_eq!( ParatensorModule::sudo_set_emission_values(<<Test as Config>::RuntimeOrigin>::root(), netuids, emission ), Err(Error::<Test>::NotSettingEnoughWeights.into()) );
});}
...
fn test_network_set_emission_ratios_fail_net() {
	new_test_ext().execute_with(|| {
        ...
        add_network(1, 0, 0);
        add_network(3, 0, 0);
        assert_eq!( ParatensorModule::sudo_set_emission_values(<<Test as Config>::RuntimeOrigin>::root(), netuids, emission ), Err(Error::<Test>::InvalidUid.into()) );
});}
...
new_test_ext().execute_with(|| { 
        let netuid: u16 = 1;
        assert_eq!(ParatensorModule::sudo_set_difficulty(<<Test as Config>::RuntimeOrigin>::root(), netuid, 120000) , Err(Error::<Test>::NetworkDoesNotExist.into()) );
});}
